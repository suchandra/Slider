...
var fdSlider = (function() {
    ...
    var  validateParameters = function(options){
    	if(options.step < 0) {
    		alert('Please enter step > 0');
    		return false;
    	}
    	if(options.max < options.min) {
    		alert('Please enter valid max and min values');
    		return false;
    	}
    	if(options.fineGradation && options.fineGradationSteps == options.step) {
    		alert('Please enter valid fineGradationSteps!!');
    		return false;
    	}
    	return true;
    };
    ...
    function fdRange(options) {
        var inp         = options.inp,
            ...
            gradationDiv   = options.gradationDiv,
            fineGradation = options.fineGradation,
            fineGradationSteps = options.fineGradationSteps,
            outerWrapper,
            innerWrapper,
            ieBlur,
            handle,
        ...
        function drawGradations() {
            ...
            if(vertical) {
                var sliderHeight = Math.max(1, (bar.offsetHeight - handle.offsetTop)) + "px";
                drawVerticalGradations(sliderHeight);
            } else {
                var sliderWidth = Math.max(1, handle.offsetLeft) + "px";
                drawHorizontalGradations(sliderWidth);            
            }           
        }
        ...
        function drawVerticalGradations(sliderHeight){
        	...
        	table.style.height = sliderHeight + "px";
        	table.className = "fd-slider-vertical vertical-range";
        	var tbody = doc.createElement("tbody");
        	var initialStep = maxRange-steps;
        	var desiredSteps = (maxRange / steps)-1;
        	var top=sliderHeight/((maxRange / steps));
        	var longRange = fineGradationSteps;
        	...
        	for(var i=0; i<desiredSteps;i++) {        		
        		...
        		top = top+(sliderHeight/((maxRange / steps)));
        ...
        function drawHorizontalGradations(sliderWidth){
        	...
        	var table = doc.createElement("table");
        	table.style.width = sliderWidth + "px";
        	table.className = "horizontal_gradation";
        	var tbody = doc.createElement("tbody");
        	var tr = doc.createElement("tr");
        	tbody.appendChild(tr);
        	var initialStep = minRange;
        	var initialStepWithoutGradation = minRange + steps;
        	if(fineGradation) {
        		var desiredSteps = (maxRange / steps)-1;
        	}
        	else {
        		var desiredSteps = (maxRange / steps);
        	}
        	
        	var left=sliderWidth/((maxRange / steps));
        	var longRange = fineGradationSteps;
        	...
        	for(var i=0; i<desiredSteps;i++) {
        		...
        		if(fineGradation) {
        			if(longRange === i+1) {
	        			...
	        			td.style.left = left+6 + "px";
	        			td.className = 'tableData horizontal_gradations';
	        			td.innerHTML = '-';
